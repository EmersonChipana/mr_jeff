...
class PrePickUpPageV2 extends StatefulWidget {
  ...
  @override
  State<PrePickUpPageV2> createState() => _PrePickUpPageStateV2();
}

class _PrePickUpPageStateV2 extends State<PrePickUpPageV2> {
      ...
      body: BlocConsumer<PickUpCubit, PickUpState>(
        listener: (ctx3, state) {
          if (state.status == PageStatus.verifying) {
            ...
            Navigator.pushNamed(ctx3, '/pickupv2');
      ...
      children: [
        GoogleMap(
          ...
          onTap: (point) {
            ...
            BlocProvider.of<PickUpCubit>(context).setCoordinates({'lat': point.latitude , 'lng':point.longitude});
            setState(() {
              _markers.clear();
  ...
  Future<void> _showLoadingDialog(BuildContext context, String title,
      String message, bool closeable) async {
    return showDialog<void>(
      context: context,
      barrierDismissible: false, // user must tap button!
      builder: (BuildContext context) {
