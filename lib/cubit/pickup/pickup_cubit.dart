...
class PickUpCubit extends Cubit<PickUpState> {
  ...
  void init() async {
    ...
    try{
      ...
      if(token != null){
        PrePickUpInfoV2Dto prePickUpInfoV2Dto = await PrePickUpServiceV2.getSchedule(0, token);
  ...
  Map<String, dynamic> mapTheRequest(String name1, String detail1) {
    ...
    String addressLink = '';
    if(state.pointerAddress != -1){
      addressId = state.prePickUpInfoV2!.address[state.pointerAddress].mrAddressId ;
      ...
      addressLink = state.prePickUpInfoV2!.address[state.pointerAddress].addressLink ;
    }
    Map<String, dynamic> maped = {
      ...
      "userId": 0,
      "timeId": state.prePickUpInfoV2!.schedule[state.pointerDate].hours[state.pointerTime].mrScheduleId,
